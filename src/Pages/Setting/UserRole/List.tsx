  ...
  return (
    <div className="col-span-4 overflow-y-scroll" style={{
      ...
      <div className="flex flex-col bg-white rounded-md shadow">
        ...
        {users.map(user => {
          return (
            <Link key={user.id} to={`/setting/user/role/${user.id}`} className={classNames("flex items-center space-x-1 justify-between hover:bg-gray-100 border-r-4 hover:border-primary-0 py-2 px-4 hover:pl-6 hover:font-medium capitalize transition-all", {
